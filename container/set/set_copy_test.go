...
func testCopyIntoIncompatibleDst(t *testing.T) {
	...
	// falls back to Push. ID 8 sits in bucket 8 for src and bucket 1
	// for dst, so it is only found if it was rehashed.
	src := testConfig().Must(testItem{ID: 1, Name: nameOne}, testItem{ID: 8, Name: nameEight})
