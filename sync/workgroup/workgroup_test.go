...
// errConcurrentCancel is the cause the stress test cancels the Group with,
// so Wait can be checked for surfacing it.
var errConcurrentCancel = errors.New("concurrent cancellation")
...
// stress test: the test the canceller reports to, the Group under test,
// the outcome counters, the ready barrier that releases the canceller
// once cancelAfter tasks have reached their selects, and the shape of the
// run: how many adders, how many tasks each enrols, plus one canceller.
...
// on a sleep landing mid-stream. A round that never forms is recorded
...
func runTestConcurrentGoAndCancel(t *testing.T) {
	...
	core.AssertErrorIs(t, fx.wg.Wait(), errConcurrentCancel, "wait")
