...
func TestWorkgroup_Integration(t *testing.T) {
	...
	t.Run("ConcurrentTasksAndCancel", func(t *testing.T) {
		...
		for {
			if tasksStarted.Load() == int32(numTasks) {
				break
			}
...
// TestGroup_NilGroups updates the test to check for errors instead of panics with Go()
func TestGroup_NilGroups(t *testing.T) {
	var wg *Group
