// Package mutex provides interfaces and utilities for synchronization primitives.
package mutex
...
// SafeLock attempts to acquire the lock on the provided mutex safely.
...
func SafeLock[T sync.Locker](mu T) (bool, error) {
	switch any(mu).(type) {
	case nil:
		return false, utils.ErrNilMutex
	default:
...
// SafeTryLock attempts to acquire the lock on the provided mutex without blocking.
...
//   - (false, ErrNilMutex) if the mutex is nil
//   - (false, err) if a panic occurred during lock attempt
...
// SafeUnlock releases the lock on the provided mutex safely.
// It handles nil mutexes and catches panics from underlying unlock operations.
// When used with ReverseUnlock or other multi-mutex operations, all mutexes
// will be attempted to be unlocked even if some unlock operations fail.
//
// Returns:
...
// SafeRLock attempts to acquire a read lock on the provided mutex safely.
// If the mutex is an RWMutex, it acquires a read lock; otherwise, it acquires an exclusive lock.
// It handles nil mutexes and catches panics from underlying lock operations.
//
// Returns:
//   - (true, nil) if the lock was successfully acquired
...
// SafeTryRLock attempts to acquire a read lock on the provided mutex without blocking.
// If the mutex is an RWMutex, it attempts a read lock; otherwise, it attempts an exclusive lock.
// It handles nil mutexes and catches panics from underlying TryLock operations.
//
// Returns:
//   - (true, nil) if the lock was successfully acquired
...
// SafeRUnlock releases a read lock on the provided mutex safely.
// If the mutex is an RWMutex, it releases a read lock; otherwise, it releases an exclusive lock.
// It handles nil mutexes and catches panics from underlying unlock operations.
// When used with ReverseUnlock or other multi-mutex operations, all mutexes
// will be attempted to be unlocked even if some unlock operations fail.
//
...
// NewSafeLockContext creates a function that safely acquires a lock with context awareness.
// The returned function will attempt to acquire a lock respecting context cancellation or timeouts.
//
// Parameters:
...
// NewSafeRLockContext creates a function that safely acquires a read lock with context awareness.
// The returned function will attempt to acquire a read lock respecting context cancellation or timeouts.
//
// Parameters:
//   - ctx: The context for cancellation or timeout control
//
...
// It handles nil mutexes, nil contexts, and catches panics from underlying lock operations.
//
// Returns:
//   - (true, nil) if the lock was successfully acquired
...
func SafeLockContext[T MutexContext](ctx context.Context, mu T) (bool, error) {
	if ctx == nil {
		return false, utils.ErrNilContext
	}
...
// If the mutex is an RWMutexContext, it acquires a read lock; otherwise, it acquires an exclusive lock.
// It handles nil mutexes, nil contexts, and catches panics from underlying lock operations.
//
// Returns:
//   - (true, nil) if the lock was successfully acquired
...
// This function will attempt to unlock all provided locks even if some unlock
// operations fail. Any errors that occur during unlocking will be aggregated
// and returned as a single error.
//
