...
func (c *Cond) checkAndAdd(skip, n int) (int32, error) {
	ptr, err := c.check(skip + 1)
	switch {
	case err != nil:
		return 0, err
	case n == 0:
...
func (c *Cond) check(skip int) (*int32, error) {
	switch {
	case c == nil:
		return nil, utils.NewNilReceiverPanic(skip+1, "Cond")
	default:
		return (*int32)(c), nil
	}
}
