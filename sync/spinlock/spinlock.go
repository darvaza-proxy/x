...
//
// Spinlocks are suitable for protecting shared resources that are accessed for very
// short periods of time. Unlike traditional mutexes that put goroutines to sleep
// when contention occurs, spinlocks actively spin (busy-wait) which can be more
// efficient when the expected wait time is very short.
//
// Usage considerations:
//   - Prefer spinlocks when lock contention is rare and lock-holding duration is minimal
//   - Avoid spinlocks for long-held locks as they waste CPU cycles while spinning
//   - Use in performance-critical code paths where context switching would be costly
//
// Example usage:
//
...
// Unlike sync.Mutex, SpinLock consumes CPU cycles while waiting to
// acquire the lock, making it more suitable for protecting
// resources that are held briefly.
...
// If the receiver is nil, it will panic with utils.ErrNilReceiver.
//
// While waiting, Lock yields the processor using runtime.Gosched()
// to allow other goroutines to execute, but it does not put the
...
//
// This method is useful in non-blocking scenarios where the caller wants
// to do something else if the lock isn't immediately available.
