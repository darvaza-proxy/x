...
func lsoffset(t time.Time) uint64 {
	...
	for i := len(leapseconds) - 1; i >= 0; i-- {
		if unixSec >= leapseconds[i].begin {
...
// utcFromTAI converts a TAI instant, expressed as a time.Time carrying
// TAI-labelled Unix seconds, to the UTC instant with the same TAI label.
// Instants inside an inserted leap second have no time.Time representation
// and map to the first second after it.
func utcFromTAI(tm time.Time) time.Time {
	off := lsoffset(tm)
	utc := tm.Add(-time.Duration(off) * time.Second)
	if off2 := lsoffset(utc); off2 != off {
		utc = tm.Add(-time.Duration(off2) * time.Second)
	}
	return utc
}
